...
class MemerizerGUI(QMainWindow):
    def __init__(self, excel_file):
        ...
        self.timer_iterval = 1000   # timer iterval, msec
        self.review_rate = 0.2      # the probablity of reviewing
    ...
    def spinBoxValueChanged(self):
        self.page_iterval = self.MainWindowUI.spinBox.value() * 1000

    def setProgressBar(self):
        max_value = self.MainWindowUI.progressBar.maximum()
        value = self.MainWindowUI.progressBar.value()
        value += int(self.timer_iterval / self.page_iterval * max_value)
        if value >= max_value:
            value = 0
            self.timer.stop()
